...
const FROM = process.env.RESEND_FROM_EMAIL || 'noreply@poramorshoo.com';
...
export async function sendPaymentConfirmedEmail(userEmail: string, expertEmail: string, scheduledAt: string) {
  await Promise.all([
    sendEmail({ to: userEmail, subject: 'পেমেন্ট সফল - সেশন কক্ষ প্রস্তুত', html: `<p>আপনার পেমেন্ট সফল হয়েছে। সেশন কক্ষ প্রস্তুত। সময়: ${scheduledAt}</p>` }),
    sendEmail({ to: expertEmail, subject: 'সেশন নিশ্চিত হয়েছে', html: `<p>সেশন নিশ্চিত হয়েছে। সময়: ${scheduledAt}</p>` }),
  ]);
}
...
export async function sendSessionReminderEmail(userEmail: string, expertEmail: string, expertName: string) {
  ...
  await Promise.all([
    sendEmail({ to: userEmail, subject: 'সেশন শুরু হতে ১ ঘণ্টা বাকি', html }),
    sendEmail({ to: expertEmail, subject: 'সেশন শুরু হতে ১ ঘণ্টা বাকি', html }),
  ]);
}
